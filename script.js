...
gorjetaInput.addEventListener("input", (evento) => {
...
function calcular(){
    ...
    if(conta !== 0 && porcentagem !== 0 && pessoas !== 0){
        ...
        const valorGorjeta = (conta * porcentagem) / pessoas
        const valorTotal = (conta + (conta * porcentagem)) / pessoas

        const strongGorjetaTotal = document.querySelector(".gorjeta-total > strong")
        strongGorjetaTotal.innerHTML = `R$ ${valorGorjeta.toFixed(2)}`

        const strongTotal = document.querySelector(".total > strong")
        strongTotal.innerHTML = `R$ ${valorTotal.toFixed(2)}`
    }
}
